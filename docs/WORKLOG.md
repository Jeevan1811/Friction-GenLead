...
- Truth: **LIVE SHEET DATA VERIFIED**, **APP BUILD VERIFIED**, **VPS DEPLOY PENDING**.
...
- The live source-data upgrade is now in place, but the application UI/API deploy is still pending. This does not supply a new real prospect/contact research provider or repair the existing SMTP `535`; no SMTP settings/password were changed. Treat those as separate external-integration/manual-email blockers, not as missing data from MSV's Excel workbooks.
- Next: commit and publish the scoped branch, promote it safely, deploy only the GenLead web/API services, then verify the live source-data endpoint/search, dashboard refresh, sync count, and PM2/HTTP health.
